export const extensionConfig = {
  id: 'kubohiroyajotaibridge',
  slug: 'jotai-bridge',
  name: 'TurboWarp-HTTP-Server-Jotai',
  description: 'TurboWarp blocks for producing Jotai bridge JSON.',
  author: 'Hiroya Kubo',
  license: 'MPL-2.0',
  unsandboxed: false,
  docsURI: 'https://kubohiroya.github.io/turbowarp-http-server-jotai/',
  blockIconURI:
    'data:image/svg+xml;base64,'
} as const;
